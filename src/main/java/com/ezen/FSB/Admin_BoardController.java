	...
	// 게시판 글 삭제
		...
		public ModelAndView deleteBoard(HttpServletRequest req, @RequestParam int board_num, @RequestParam Map<String, String> params) {
			...
			if(view.equals("secondhand")) {
				mem_num = loginMapper.shBoardNum(board_num);
				title = loginMapper.shBoardTitle(board_num);
			}else {
				mem_num = loginMapper.BoardNum(board_num);
				 title = loginMapper.BoardTitle(board_num);
			}
			// 받은 img 이름
		...
		public ModelAndView deleteBoardReply(HttpServletRequest req, @RequestParam int br_num, @RequestParam int board_num, @RequestParam String view) {
			...
			if(view.equals("secondhand")) { // 중고 게시판 댓글
				...
				if(list.size() >0) { // 신고내역이 있다면
					...
					// 알람처리 중고댓글 
					int mem_num = loginMapper.shBoardNumReply(br_num);
					int boardNum = loginMapper.shBoardTitleReply(br_num);
					String title = loginMapper.BoardTitle(boardNum);
					AlarmDTO rm = new AlarmDTO();
					rm.setMem_num(mem_num);
					rm.setAlm_cate("신고");
				...
				if(list.size()>0) { // 신고내역이 있다면
					...
					// 알람처리 게시판댓글 
					int mem_num = loginMapper.BoardNumReply(br_num);
					int boardNum = loginMapper.BoardTitleReply(br_num);
					String title = loginMapper.shBoardTitle(boardNum);
					AlarmDTO rm = new AlarmDTO();
					rm.setMem_num(mem_num);
					rm.setAlm_cate("신고");
