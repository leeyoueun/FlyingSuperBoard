 ...
 */
	...
	public ModelAndView login_ok(HttpServletRequest req, HttpServletResponse resp) {
		...
		//				session.setAttribute("member", login_mem); // 다원 디티오 통째 추가 
						...
						// 안읽은 알람
						List<AlarmDTO> listCount = loginMapper.boardunReadAlarmList(login_mem.getMem_num());
						//총 알람의 갯수
...
//		req.setAttribute("url", url);
	 ...
	 @RequestMapping(value = "/kakao.do", produces = "application/json")
		public ModelAndView kakaoLogin(@RequestParam("code") String code, HttpServletRequest request,
			...
			if(id_check==null) {
				...
				int res2= loginMapper.kakaoMemNum();
				System.out.println("카카오 로그인 멤버 번호" + res2);
				dto.setMem_num(res2);
				
				int res = loginMapper.insertKakaoMember(dto);
			...
			System.out.println("야야야!!="+accessToken);
			session.setMaxInactiveInterval(86400);
			...
			if(id_check == null) { // 신규 회원
				...
				// 안읽은 알람
				listCount = loginMapper.boardunReadAlarmList(dto2.getMem_num());
			}else { // 기존 회원
				msg = id_check.getMem_name() + "님, 로그인 되었습니다.";
				// 안읽은 알람
				listCount = loginMapper.boardunReadAlarmList(id_check.getMem_num());
			}
	...
	public String unlink(HttpSession session, HttpServletRequest request) {
		...
		System.out.println("제이슨의 억세스토큰"+access_Token);
		String jsonString = access_Token.toString();
		System.out.println("억세스토큰 알렺룬다!!!"+jsonString);
