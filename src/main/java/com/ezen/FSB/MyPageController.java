...
//	private BoardMapper boardMapper;
	...
	public String SaveImg(HttpServletRequest req, HttpSession session) throws IllegalStateException, IOException {
		...
		MemberDTO dto = (MemberDTO) session.getAttribute("login_mem");
		System.out.println("야 이거 ㄴ아ㅘ라=" + dto.getMem_nickname());
		// 이미지 받기
...
//	}
	...
	public String changePwOk(HttpServletRequest req) {
		...
		System.out.println("감자의 들어온 비미번호="+passwd);
		System.out.println("감자의 들어온 실제 번호="+dto.getMem_passwd());
	...
	public String nickChangeOK(HttpServletRequest req, HttpSession session) {
		...
		dto.setMem_id(id);
		int res = loginMapper.changeNick(dto);
		...
		int res2 = loginMapper.updateNicknameProfile(pdto);
		if(res2>0) {
	...
	public String changePhoneSMS(@RequestParam Map<String, String> params, HttpServletRequest req) {
		...
		System.out.println("번호랑 아이디 넘"+phoneNumber+id+numStr);
	...
	public String changePhoneSMS_change(@RequestParam Map<String, String> params, HttpServletRequest req, HttpSession session) {
		...
		int res = loginMapper.changePhone(dto);
	...
	public String changeBadge(HttpServletRequest req, HttpSession session) {
		...
		req.setAttribute("id", id);
		 ...
		 int boardCount = loginMapper.memCountBoard(num);
		 int replyCount = loginMapper.memCountReply(num);
		 ...
		 int likeCount = loginMapper.memCountLike(num);
		 int shopCount = loginMapper.memCountShop(num);
		 ...
		 int res = loginMapper.changeBadge(dto);
	...
	public String changeBadgeOk(HttpServletRequest req, HttpSession session) {
		...
		MemberDTO dto = loginMapper.findMember(id);
		System.out.println("check1="+req.getParameter("check1"));
		System.out.println("check1="+req.getParameter("check2"));
		System.out.println("check1="+req.getParameter("check3"));
		System.out.println("check1="+req.getParameter("check4"));
		System.out.println("check1="+req.getParameter("check5"));
		...
		int res = loginMapper.changeBadgeOk(dto);
	...
	public String changeTag(HttpServletRequest req, HttpSession session) {
		...
		System.out.println("tag"+dto.getTag_1()+dto.getTag_2()+dto.getTag_3()+dto.getTag_4());
	...
	public String changeTagOk(HttpServletRequest req, HttpSession session) {
		...
		int res = loginMapper.changeTag(dto);
	...
	public String myBoard(HttpServletRequest req, HttpSession session) {
		...
		//1. 글 수
		int shboard = loginMapper.memCountShBoard(num);
		int freeboard = loginMapper.memCountFreeBoard(num);
		int secretboard = loginMapper.memCountSecretBoard(num);
		int allboard = shboard+freeboard+secretboard ;
		
		java.util.Map<String, Integer> params = new HashMap<>();
		params.put("num", num);
		//2. 댓글 수
		List<BoardDTO> list = loginMapper.freeReply(params);
		int f_list = list.size();
		
		List<BoardDTO> list2 = loginMapper.secretReply(params);
		int sc_list = list2.size();
		
		List<BoardDTO> list3 = loginMapper.shReply(params);
	...
	//자유게시판 리스트 
		...
		public ModelAndView myFreeBoard(HttpServletRequest req, java.util.Map<String, Integer> params) {
			...
			int count = loginMapper.memCountFreeBoard(num);
			...
			if (count > 0) {
				list = loginMapper.MemfreelistBoard(params);
		...
		//중고 게시판 리스트 
				...
				public ModelAndView myShBoard(HttpServletRequest req, java.util.Map<String, Integer> params) {
					...
					if (count > 0) {
						list = loginMapper.MemShlistBoard(params);
						System.out.println("리스트="+list.get(0));
						int pageCount = (count / pageSize) + (count % pageSize == 0 ? 0 : 1);
						int pageBlock = 2;
						int startPage = (currentPage - 1) / pageBlock * pageBlock + 1;
				...
				public ModelAndView mySecretBoard(HttpServletRequest req, java.util.Map<String, Integer> params) {
					...
					if (count > 0) {
						list = loginMapper.MemSecretlistBoard(params);
						System.out.println("리스트="+list.get(0));
						int pageCount = (count / pageSize) + (count % pageSize == 0 ? 0 : 1);
						int pageBlock = 2;
						int startPage = (currentPage - 1) / pageBlock * pageBlock + 1;
				...
				public ModelAndView myFreeReply(HttpServletRequest req, java.util.Map<String, Integer> params) {
					...
					if (count > 0) {
						list = loginMapper.freeReply(params);
						System.out.println("리스트="+list.get(0));
						int pageCount = (count / pageSize) + (count % pageSize == 0 ? 0 : 1);
						int pageBlock = 2;
						int startPage = (currentPage - 1) / pageBlock * pageBlock + 1;
				...
				public ModelAndView myShReply(HttpServletRequest req, java.util.Map<String, Integer> params) {
					...
					if (count > 0) {
						list = loginMapper.shReply(params);
						System.out.println("리스트="+list.get(0));
						int pageCount = (count / pageSize) + (count % pageSize == 0 ? 0 : 1);
						int pageBlock = 2;
						int startPage = (currentPage - 1) / pageBlock * pageBlock + 1;
				...
				@RequestMapping("/mySecretReply.do")
				public ModelAndView mySecretReply(HttpServletRequest req, java.util.Map<String, Integer> params) {
					...
					if (count > 0) {
						list = loginMapper.secretReply(params);
						System.out.println("리스트="+list.get(0));
						int pageCount = (count / pageSize) + (count % pageSize == 0 ? 0 : 1);
						int pageBlock = 2;
						int startPage = (currentPage - 1) / pageBlock * pageBlock + 1;
				...
				@RequestMapping("/dontgo.do") //  핸드폰 번호 변경 버튼 클릭시
				public String dontgo() {
					return "mypage/dontgo";
				}
				
				@RequestMapping("/dontgoplz.do") //  핸드폰 번호 변경 버튼 클릭시
				public String dontgoplz(HttpServletRequest req, HttpSession session, @ModelAttribute DontgoDTO dto2) {
					int res = loginMapper.reason(dto2);
					
					return "mypage/dontgoplz";
				}
				
				@RequestMapping("/dontgoplzOk.do") //  핸드폰 번호 변경 버튼 클릭시
				public String dontgoplzOk(HttpServletRequest req, HttpSession session) {
					...
					System.out.println("적은 패스워드="+writePasswd+"ㅈ실베비번"+realPasswd);
					if (realPasswd.equals(writePasswd)){
						int res = loginMapper.deleteMem(dto.getMem_id());
				...
				// 좋아요한 보드게임 목록
					...
					public ModelAndView gameLikeList(HttpServletRequest req) {
						...
						List<GameDTO> list = loginMapper.myPageGameLikeList(dto.getMem_num());
					...
					public ModelAndView myCoupon(HttpServletRequest req) {
						...
						// 비즈니스 쿠폰 만료일 삭제
				         loginMapper.expireBcouponUser();
				         
						List<BusinessCouponUserDTO> list = loginMapper.myPageCouponList(dto.getMem_num());
					...
					public ModelAndView couponDel(HttpServletRequest req) {
						...
						int res = loginMapper.delCoupon(num);
					...
					public ModelAndView myAlarm(HttpServletRequest req) {
						...
						// 새 알람이 등록됐을
						List<AlarmDTO> list = loginMapper.boardAlarmList(dto.getMem_num());
						
						// 안읽은 알람
						List<AlarmDTO> listCount = loginMapper.boardunReadAlarmList(dto.getMem_num());
						//총 알람의 갯수
					...
					public ModelAndView alarmDel(HttpServletRequest req) {
						...
						//삭제하기
						int res = loginMapper.delAlarm(num);
...
//						dto3.setAlm_read(1);
						int res = loginMapper.readAlarm(num);
						
						List<AlarmDTO> list = loginMapper.boardunReadAlarmList(dto.getMem_num());
