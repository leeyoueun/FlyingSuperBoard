...
//		}      
	...
	public ModelAndView shop_checkFind(@RequestParam(required=false, name="theme") List<String> theme, @RequestParam(required=false, name="game_player") List<String> game_player, @RequestParam(required=false, name="prod_starrating") List<String> prod_starrating) {
		ModelAndView mav = new ModelAndView("shop/shop_main");
		...
		if(theme == null) {
			...
			for(String t : theme) {
			    ...
			    if(prod_starrating == null) {
			         
			    } else if (prod_starrating.size() != 0) {
			       for(String s : prod_starrating) {
			          System.out.println("별점"+s);
	...
	public ModelAndView prod_find(HttpServletRequest req, String search, String searchString) {
		...
		return new ModelAndView("shop/shop_main", "listProd", list);
	}
	
	//쇼핑몰 상품상세 1.상세페이지
	@RequestMapping("/shop_view.do")
	public ModelAndView viewShop(HttpServletRequest req, @RequestParam Map<String, Integer> params, @RequestParam int prod_num) {
...
//				mav.addObject("prod_num", prod_num);
	...
	public ModelAndView deleteLike(HttpServletRequest req, @RequestParam int prod_num, @RequestParam String mode, @RequestParam Map<String, Integer> params) {
		...
		if(mode.equals("listLike")) {
			...
			return new ModelAndView("shop/shop_listLike");
		}
	...
	//쇼핑몰 상품상세 2.포토리뷰 전체보기
	@RequestMapping("shop_view2_photo.do")
	public ModelAndView listPhotoReview(HttpServletRequest req, @RequestParam Map<String, Integer> params,  @RequestParam int prod_num) {
		return new ModelAndView("shop/shop_view2_photo");
	...
	public ModelAndView cartOrder(HttpServletRequest req, @RequestParam int prod_num, @RequestParam int cart_qty) {
...
//		ddto.setMem_num(mem_num);	// 배송지 테이블
		...
		if (mode.equals("view")) {
			...
			odto.setOrder_receiptprice(odto.getOrder_price() - odto.getOrder_coupon() - odto.getOrder_coupon());
		...
		if (mode.equals("cart")) {		
			...
			int order_num_Seq = shopMapper.order_num_Seq();
			odto.setOrder_num(order_num_Seq);
			odto.setOrder_receiptprice(odto.getOrder_receiptprice() - odto.getOrder_coupon() - odto.getOrder_point());
			shopMapper.insertOrder(odto);
		...
		String order_point =(String) params.get("order_point");
		if(Integer.parseInt(order_point) > 0) {
		phdto.setPoint_type((String) params.get("point_type_use"));//-사용
		phdto.setPoint_content((String) params.get("point_content_use"));//사용내용
		
		System.out.println(params.get("order_point"));
			...
			phdto.setPoint_total(shopMapper.userPointTotal(mem_num) - Integer.parseInt(o));
			shopMapper.pointOrder(phdto);
		}
...
//		shopMapper.insertDel(ddto);
	...
	public ModelAndView s(HttpServletRequest req, @RequestParam int order_num, @RequestParam Map<String, Integer> params) {
		...
		List<ShopOrderDTO> list = shopMapper.listOrderDetailVeiw(params);
